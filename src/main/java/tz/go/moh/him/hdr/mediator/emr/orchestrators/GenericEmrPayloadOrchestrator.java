            ...
            if (objects.size() > 0) {
                ...
                switch (emrPayload.getMessageType()) {
                    case "BEDOCC":
                        ...
                        break;
                    case "DDC":
                        actor = getContext().actorOf(Props.create(DailyDeathCountOrchestrator.class, config));
                        log.info("Forwarding request to: " + DailyDeathCountOrchestrator.class.getSimpleName());
                        break;
                    case "REV":
